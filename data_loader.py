...
def save_caption_vectors_birds(data_dir):
	import time
	...
	for image_folder in image_folders:
		...
		image_files.extend([f for f in os.listdir(img_dir) if 'jpg' in f])
	print(image_files[300:400])
	print(len(image_files))
	image_captions = { img_file : [] for img_file in image_files }

	caption_dir = join(data_dir, 'birds/text_c10')
	class_folders = [f for f in os.listdir(caption_dir)]

	class_dirs = []
	for f in class_folders:
		class_dirs.append( join(caption_dir, f))
...
def main():
	...
	parser.add_argument('--batch_size', type=int, default=64,
                       help='Batch Size')
	parser.add_argument('--data_set', type=str, default='flowers&birds',
                       help='Data Set : Flowers, MS-COCO')
	args = parser.parse_args()

	if args.data_set == 'flowers':
		save_caption_vectors_flowers(args.data_dir)
	elif args.data_set == 'birds':
		save_caption_vectors_birds(args.data_dir)
	elif args.data_set == 'flowers&birds':
		save_caption_vectors_flowers(args.data_dir)
		save_caption_vectors_birds(args.data_dir)
	else:
